...
func (s *Server) handleConnections(w http.ResponseWriter, r *http.Request) {
        ...
        for {
                ...
                case TypeLogin:
                        ...
                        log.Printf("User %s logged in", msg.Username)

                        joinMsg := ChatMessage{
                                Type:      TypeMessage,
                                Content:   fmt.Sprintf("%s has joined the chat", msg.Username),
                                Username:  "System",
                                Timestamp: time.Now().UTC().Format(time.RFC3339),
                        }
                        s.broadcast <- joinMsg
...
func (s *Server) sendChatHistory(client *websocket.Conn) error {
        ...
        // Definim o structură temporară pentru a decoda datele brute din MongoDB
        type RawChatMessage struct {
                Type      MessageType `bson:"type"`
                Username  string      `bson:"username"`
                Content   string      `bson:"content"`
                Timestamp interface{} `bson:"timestamp"` // Acceptăm orice tip (string sau time.Time)
        }
        ...
        // Convertim timestamp-urile în string ISO
        ...
        for _, raw := range rawMessages {
                ...
                case string:
                        timestampStr = t
                case time.Time:
                        timestampStr = t.UTC().Format("2006-01-02T15:04:05.999Z") // Matches JavaScript's toISOString()
                default:
...
func main() {
        ...
        if config.MongoURI == "" {
                config.MongoURI = "mongodb://mongodb-service:27017/chat"
        }
